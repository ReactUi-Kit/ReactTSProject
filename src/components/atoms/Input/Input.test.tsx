...
test("renders input with min/max attributes for number type", () => {
  ...
  const inputElement = screen.getByPlaceholderText(inputProps.placeholder);
  expect(inputElement).toHaveAttribute("min", inputProps.minLength.toString());
  expect(inputElement).toHaveAttribute("max", inputProps.maxLength.toString());
});
...
test("handles input changes", () => {
  ...
  // Test that the input is initially empty
  const inputElement = screen.getByPlaceholderText(inputProps.placeholder);
...
test("renders the input with the correct type", () => {
  const inputProps = {
    type: "email",
  ...
  // Test that the input type is email
  const inputElement = screen.getByPlaceholderText(inputProps.placeholder);
